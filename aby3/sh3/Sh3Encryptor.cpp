...
namespace aby3
{
    ...
    {
        return dep.then([this, & sour, & dest](CommPkg& comm, Sh3Task& self) {
            comm.mNext.asyncSendCopy(sour[0]);
            dest[0] = 0;
            auto fu = comm.mPrev.asyncRecv(dest[1]);
    ...
    //part of FSS delta reveal: p1 the second data to p0 the first data
    Sh3Task Sh3Encryptor::CovertN2PBinary(Sh3Task dep, sb64 &sour, sb64 & dest)
    {
        return dep.then([this, & sour, & dest](CommPkg& comm, Sh3Task& self) {
            comm.mPrev.asyncSendCopy(sour[1]);
            dest[1] = 0;
            auto fu = comm.mNext.asyncRecv(dest[0]);
...
//            oc::lout << self.mRuntime->mPartyIdx << " localIntMatrix 1" << std::endl;
            ...
            for (i64 i = 0; i < ret.mShares[0].size(); ++i) {
                ret.mShares[0](i) = mShareGen.getShare() + m(i);
//                std::cout << "ret.mShares[0](i) : " << ret.mShares[0](i) << std::endl;
            }

//            std::cout << "ret.mShares[0].size(): " << ret.mShares[0].size() << std::endl;
            comm.mNext.asyncSendCopy(ret.mShares[0].data(), ret.mShares[0].size());
//            comm.mNext.send(ret.mShares[0].data(), ret.mShares[0].size());
            auto fu = comm.mPrev.asyncRecv(ret.mShares[1].data(), ret.mShares[1].size());
//            comm.mPrev.recv(ret.mShares[1].data(), ret.mShares[1].size());

            self.then([fu = std::move(fu)](CommPkg& comm, Sh3Task& self) mutable {
//             self.then([&fu](CommPkg& comm, Sh3Task& self) mutable{
//                oc::lout << self.mRuntime->mPartyIdx << " localIntMatrix 2" << std::endl;
                if (fu.valid()) {
//                    oc::lout << self.mRuntime->mPartyIdx << " call get()" << std::endl;
                    fu.get();
                } else
                    std::cout << "==========fu error"  << std::endl;
//                oc::lout << self.mRuntime->mPartyIdx << " localIntMatrix 2 ok" << std::endl;
            });

//            std::this_thread::sleep_for(std::chrono::milliseconds(100));
//            std::cout << "ret.mShares[1](i) : " << ret.mShares[1](0) << std::endl;
        }).getClosure();
...
//                oc::lout << self.mRuntime->mPartyIdx << " remoteIntMatrix 2" << std::endl;
                fu.get();
//                oc::lout << self.mRuntime->mPartyIdx << " remoteIntMatrix 2 ok" << std::endl;
            });
        }).getClosure();
    }
    ...
    Sh3Task Sh3Encryptor::localBinMatrix(Sh3Task dep, const i64Matrix & m, sbMatrix & ret)
    {
        return dep.then([this, &m, &ret](CommPkg& comm, Sh3Task self) {
            ...
            if (b0 || b1)
                throw std::runtime_error(LOCATION);
            // std::cout << "ret.mShares[1].size(): " << ret.mShares[1].size() << "\n";
            for (u64 i = 0; i < ret.mShares[0].size(); ++i){
