...
namespace aby3
{
    ...
    //}
    Sh3Task Sh3Evaluator::asyncAnd(Sh3Task dependency, const sb64& A, const sb64& B, sb64& C)
    {
    ...
    Sh3Task Sh3Evaluator::asyncMul(Sh3Task dependency, const sb64 &A, const sb64 &B, sb64 &C) {
        return dependency.then([&](CommPkg &comm, Sh3Task self) {
            // convert to GF(2^k) poly
            if (mPartyIdx == 999) {
                GF2X A_0 = i64_GF2X(A[0]);
                GF2X B_0 = i64_GF2X(B[0]);
                cout << "degree: " << GF2E::degree() << endl;
                cout << "degree: " << GF2E::modulus() << endl;
                cout << "A[0]: " << A_0 << endl;
                cout << "B[0]: " << B_0 << endl;

                cout << "GF2E-A[0]: " << to_GF2E(A_0) << endl;
                cout << "GF2E-B[0]: " << to_GF2E(B_0) << endl;
                cout << "GF2E-A*B: " << I642POLY(A[0])*I642POLY(B[0]) << endl;

                GF2E c0 = I642POLY(A[0])*I642POLY(B[0]);
                GF2E c1 = I642POLY(A[0])*I642POLY(B[1]);
                GF2E c2 = I642POLY(A[1])*I642POLY(B[0]);
                cout << "GF2E-A*B1: " << c0+c1+c2 << endl;
                cout << "GF2E-A*B1: " << POLY2I64(c1) << endl;

            }
            GF2E c0
                 = I642POLY(A[0]) * I642POLY(B[0])
                 + I642POLY(A[0]) * I642POLY(B[1])
                 + I642POLY(A[1]) * I642POLY(B[0]);
//                 + I642POLY(mShareGen.getShare());
...
//                      + mShareGen.getShare();
            if (mPartyIdx == 999) {
                cout << "GF2E-A*B1: " << POLY2I64(c0) << endl;
                cout << "GF2E-A*B1 ret: " << C[0] << endl;
            }
            comm.mNext.asyncSendCopy(C[0]);
            auto fu = comm.mPrev.asyncRecv(C[1]).share();
